...
packages = c("downloader","haven","stars","dplyr","sf","rdhs", "classInt", "readr", "ggplot2", "here", "s2", "foreign")
...
lhzSensitivity = lhz %>% 
  ...
  mutate(
    ...
    disasterCoping = percent_rank(desc(jPctDCS)) * 4 + 1, #bc higher income from ecologically destructive practices makes you more vulnerable
    cashCropInc = percent_rank(desc(jPctIncCas) * 4 + 1 #bc selling cash crops makes you vulnerable to the global economy
  ) %>% 
    rowwise %>% 
    mutate(
      sensitivity = sum(
  ...
  ```{r saving livelihood sensitivity socres}
  
  write_sf(
    lhzSensitivity, here("data", "derived", "private", "lhzSensitivity.shp")
  )
